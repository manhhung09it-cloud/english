...
class GooglePayHandler extends AlternativeBaseHandler {
    ...
    async getGooglePaymentDataRequest() {
        ...
        this.payPalOrder = await this.createPaypalOrder(this.jsShoppingCart.createOrderUrl, []);
        let paymentDataRequest = Object.assign({}, baseRequest);
        paymentDataRequest.transactionInfo = this.getGoogleTransactionInfo(countryCode, this.payPalOrder);
    ...
    onPaymentAuthorized(paymentData) {
        this.processPayment(paymentData).then((result) => {
            ...
            console.log(error);
            this.handleLockOrUnlockGooglePayButton('unlock')
        });
    }
        ...
        if (this.paymentsClient === null) {
            this.paymentsClient = new this.googlePaySDK.payments.api.PaymentsClient({
                ...
                paymentDataCallbacks: {
                    onPaymentAuthorized: (paymentData) => {
                        this.handleLockOrUnlockGooglePayButton('lock')
     ...
     */
    getGoogleTransactionInfo(countryCode, order) {
        return {
            countryCode: countryCode,
            currencyCode: order.orderData.currencyCode.toString(),
            totalPriceStatus: "FINAL",
            totalPrice: order.orderData.totalAmount.toString()
        };
    }
    ...
    async onGooglePaymentButtonClicked() {
        this.handleLockOrUnlockGooglePayButton();
    ...
    handleLockOrUnlockGooglePayButton(status = 'lock')
    {
        let googlePayButton = this.getGooglePayButton();
        switch(status) {
            case 'lock':
